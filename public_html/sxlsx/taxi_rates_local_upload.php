...
$xlsx = new SimpleXLSX('taxi_rates_local.xlsx');
...
foreach( $xlsx->rows() as $r ) 
{
	...
	$operator_name = $r[2];
	$min_hour_per_booking = $r[4];
	$min_fare_per_booking = $r[5];
	$max_km_per_min_hour = $r[6];
	$additional_rate_per_hour = $r[7];
	$additional_km_per_hour = $r[8];
	$additional_rate_per_km = $r[9];
	...
	if($city_id && $taxi_model_id && $operator_id)
	{
		...
		$sql = mysqli_query($db, "SELECT id from taxi_rates_local where taxi_model_id = '$taxi_model_id' and $city_id = '$city_id' and operator_id = '$operator_id'");
		if(mysqli_num_rows($sql) > 0)  
		{
			$rlt = mysqli_fetch_array($sql,MYSQL_ASSOC);
			$rate_id = $rlt['id'];
			mysqli_query($db,"UPDATE taxi_rates_local set min_hour_per_booking = '$min_hour_per_booking', min_fare_per_booking='$min_fare_per_booking', max_km_per_min_hour = '$max_km_per_min_hour', additional_rate_per_hour = '$additional_rate_per_hour', additional_km_per_hour = '$additional_km_per_hour', additional_rate_per_km = '$additional_rate_per_km' where id = '$rate_id'");
			$updated++;
		}
		else
		{
			mysqli_query($db, "INSERT INTO taxi_rates_local (operator_id, taxi_model_id, taxi_type_id, city_id, min_hour_per_booking, min_fare_per_booking, max_km_per_min_hour, additional_rate_per_hour, additional_km_per_hour, additional_rate_per_km) values ($operator_id, $taxi_model_id, $taxi_type_id,$city_id,'$min_hour_per_booking', '$min_fare_per_booking' , '$max_km_per_min_hour' , '$additional_rate_per_hour' ,'$additional_km_per_hour','$additional_rate_per_km')");
