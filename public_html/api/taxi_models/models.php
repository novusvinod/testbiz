	...
	class API extends REST 
	{
		...
		*/
		...
		public function getAll()
		{
			...
			try
			{
				$query = "SELECT tm.id `model_id`, concat(cc.name, ' ',tm.name) `taxi_model`, tm.car_company `car_company_id`, cc.name `car_company`, tm.type_id, tt.name `taxi_type` from taxi_models `tm` inner join taxi_types `tt` on tm.type_id = tt.id inner join car_companies `cc` on tm.car_company = cc.id";
				$sql = mysqli_query($this->db, $query);
				if(mysqli_num_rows($sql) > 0)
				{
					$result = array();
