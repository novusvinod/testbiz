...
$xlsx = new SimpleXLSX('AIRPORTs.xlsx');
...
foreach( $xlsx->rows() as $r ) 
{
	...
	$sql = mysqli_query($db, "SELECT id from airports where name = '$name'");
	if(mysqli_num_rows($sql) > 0)  
	{
	...
	else
	{
		if($city_id)
		{
			mysqli_query($db, "INSERT INTO airports (name, city_id, latitude, longitude, created) values ('$name', '$city_id', '$latitude','$longitude', now())");
