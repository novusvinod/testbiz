<?php
	...
	//----------------------------------COMPANY - Profile------------------------------//
		...
		private function assignRadioBooking()
		{
			...
			{
				mysqli_query($this->db, "UPDATE bookings SET 
					external_reference_no='$reference_no', operator_name='$operator_name', driver_name='$driver_name',
...
//*********************************************************************************************************************//
			...
			try
			{
				...
				if(!empty($username) and !empty($password))
				{
					if(filter_var($username, FILTER_VALIDATE_EMAIL))
					{
						...
						$sql = mysqli_query($this->db, "SELECT u.*, a.has_auth_level, a.has_assessment_codes from users `u` left join admins `a` on u.admin_id=a.id  where u.username = '$username' AND u.password = '$enc_pwd'");
			...
			try
			{
				...
				if($tour_type == '0')
				{
					$query = "INSERT into bookings (tour_type,pickup_location,drop_location,
						...
						'$admin_id','$group_id', '$subgroup_id',
						'$ass_code', '$reason_booking', '$no_of_seats',
						'$auth_1_status', '$status_auth2', '$auth_accept_timing')";
			...
			try
			{
				...
				if(!empty($username) and !empty($password))
				{
					if(filter_var($username, FILTER_VALIDATE_EMAIL))
					{
						...
						$sql = mysqli_query($this->db, "SELECT sa.*, sg.budget, sg.is_radio, sg.is_local, sg.is_outstation, sg.is_bus, a.has_auth_level, a.has_assessment_codes from subgroup_authenticater `sa` left join subgroups `sg` on sa.subgroup_id = sg.id left join admins `a` on sa.admin_id=a.id where sa.email = '$username' AND sa.password = '$enc_pwd' LIMIT 1");
			...
			try
			{
				if(!empty($username) and !empty($password))
				{
					if(filter_var($username, FILTER_VALIDATE_EMAIL))
					{
						...
						$sql = mysqli_query($this->db, "SELECT ga.*, g.budget, g.is_radio, g.is_local, g.is_outstation, g.is_bus, a.has_auth_level, a.has_assessment_codes from group_authenticater `ga` left join groups `g` on ga.group_id = g.id left join admins `a` on g.admin_id=a.id where ga.email = '$username' AND ga.password = '$enc_pwd' LIMIT 1");
			...
			try
			{
				...
				switch($type)
				{
					case '1':  //Active (Unassigned)
					{
						...
						from bookings `b`
						left join groups `g` on b.group_id = g.id 
						left join admins `a` on g.admin_id = a.id where b.status_user = 1 and (b.status_auth1 = 1 OR b.status_auth2 = 1) and b.status_auth_taxivaxi < 2 order by pickup_datetime asc";
						break;
					}
					case '3':  //Archived
					{
		...
		private function getAllTaxivaxiBusBookings()
		{
			...
			try
			{
				...
				switch($type)
				{
					case '1': //Active Unassigned
					{
						$query  = "SELECT b.*,u.user_cid,u.user_name,u.user_contact,u.email,p.people_cid,p.people_name,p.people_email,p.people_contact, a.corporate_name from bus_bookings `b` inner join users `u` on b.user_id = u.id inner join people `p` on b.people_id = p.id inner join admins `a` on b.admin_id=a.id where ((b.status_auth1 = 1 or b.status_auth2 = 1) and b.status_spoc = 1 and b.status_auth_taxivaxi < 2) and b.is_assign = 0 order by b.time_range asc";
						break;
					}
					case '3': //Archived
					{
						$query  = "SELECT b.*,u.user_cid,u.user_name,u.user_contact,u.email,p.people_cid,p.people_name,p.people_email,p.people_contact, a.corporate_name from bus_bookings `b` inner join users `u` on b.user_id = u.id inner join people `p` on b.people_id = p.id inner join admins `a` on b.admin_id=a.id where b.status_spoc = 1 and (b.status_auth1 = 1 OR b.status_auth2 = 1) and b.status_auth_taxivaxi = 3 and b.is_invoice = 0 and b.date_of_journey < DATE_SUB(NOW(), INTERVAL 24 HOUR) order by b.time_range asc";
						break;
					}
					case '4': //Rejected
					{
						$query  = "SELECT b.*,u.user_cid,u.user_name,u.user_contact,u.email,p.people_cid,p.people_name,p.people_email,p.people_contact, a.corporate_name from bus_bookings `b` inner join users `u` on b.user_id = u.id inner join people `p` on b.people_id = p.id inner join admins `a` on b.admin_id=a.id where b.status_spoc = 2 OR b.status_auth1 = 2 OR b.status_auth2 = 2 OR b.status_auth_taxivaxi = 2  order by b.time_range asc";
						break;
					}
					case '2': //Active Assigned
					{
						$query  = "SELECT b.*,u.user_cid,u.user_name,u.user_contact,u.email,p.people_cid,p.people_name,p.people_email,p.people_contact, a.corporate_name from bus_bookings `b` inner join users `u` on b.user_id = u.id inner join people `p` on b.people_id = p.id inner join admins `a` on b.admin_id=a.id where ((b.status_auth1 = 1 or b.status_auth2 = 1) and b.status_spoc = 1 and b.status_auth_taxivaxi = 3 and b.is_invoice = 0 and b.date_of_journey > DATE_SUB(NOW(), INTERVAL 24 HOUR)) order by b.time_range asc";
						break;
					}
		...
		private function assignTaxivaxiBusBooking()
		{
			...
			$boarding_point_taxivaxi = mysqli_real_escape_string($this->db,$this->_request['boarding_point_taxivaxi']);
			$pickup_datetime_taxivaxi = $this->_request['pickup_datetime_taxivaxi'];
			$ticket_number = mysqli_real_escape_string($this->db,$this->_request['ticket_number']);
			$pnr_number = mysqli_real_escape_string($this->db,$this->_request['pnr_number']);
			$bus_type_allocated = $this->_request['bus_type_allocated'];
			$operator_name = mysqli_real_escape_string($this->db,$this->_request['operator_name']);
		...
		private function getAllTaxivaxiadminBusInvoices()
		{
			...
			try
			{
				...
				if(mysqli_num_rows($sql) > 0)
				{
					...
					while($rlt = mysqli_fetch_array($sql, MYSQL_ASSOC))
					{
						/*if($rlt['id'] == '944' || $rlt['id'] == '934')
							continue;*/
						$rlt['operator_name'] = $this->clean($rlt['operator_name']);
						$rlt['reason_of_booking'] = $this->clean($rlt['reason_of_booking']);
						$rlt['boarding_point'] = $this->clean($rlt['boarding_point']);
						$rlt['boarding_point_taxivaxi'] = $this->clean($rlt['boarding_point_taxivaxi']);
						//array_push($result, $rlt);
						$result[] = $rlt;
					}
					$result = array('access_token' => $token_id,'Bookings'=>$result);
		...
		private function addAssessmentCode()
		{
			$token_id = $this->_request['access_token'];	
			$admin_id = $this->checkAdmin($token_id);
			$assessment_code = mysqli_real_escape_string($this->db,$this->_request['assessment_code']);
			$code_desc = mysqli_real_escape_string($this->db,$this->_request['code_desc']);
