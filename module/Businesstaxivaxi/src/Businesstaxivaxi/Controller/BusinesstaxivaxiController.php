...
define ( 'BUSINESSTAXIVAXIADDCOMPANY', 'http://taxivaxi.in/business/api/addCompany' );
...
//Operatons
define ( 'OPERATORS', 'http://taxivaxi.in/business/api/getAllOperators' );
define ( 'ASSIGNOPERATOR', 'http://taxivaxi.in/business/api/assignOperator' );
define ( 'ADDASSIGNOPERATOR', 'http://taxivaxi.in/business/api/addAndAssignOperator' );

//Report Operator Issue
define ( 'REPORTOPERATORISSUE', 'http://taxivaxi.in/business/api/reportOperatorIssue' );
...
class BusinesstaxivaxiController extends AbstractActionController
{
    ...
    public function getBusinessadminAddcompanyAction()
    {
        if(isset($_COOKIE['business_taxivaxi']))
        {
            return new ViewModel();
    ...
    public function postBusinessadminAddcompanyAction()
    {
        $request = $this->getRequest ();

        $data['corporate_name'] = $request->getPost('corporate_name');
        $data['corporate_code'] = $request->getPost('corporate_code');
        $data['contact_name'] = $request->getPost('contact_name');
        $data['contact_no'] = $request->getPost('contact_no');

        $data['email'] = $request->getPost('email');
        $data['password'] = $request->getPost('password');
        $data['billing_name'] = $request->getPost('billing_name');
        $data['billing_address_line1'] = $request->getPost('billing_address_line1');

        $data['billing_address_line2'] = $request->getPost('billing_address_line2');
        $data['billing_address_line3'] = $request->getPost('billing_address_line3');
        $data['has_auth_level'] = $request->getPost('has_auth_level');
        $data['has_billing_spoc_auth_level'] = $request->getPost('has_billing_spoc_auth_level');

        $data['has_billing_admin_auth_level'] = $request->getPost('has_billing_admin_auth_level');
        $data['has_assessment_codes'] = $request->getPost('has_assessment_codes');
        $data['is_radio'] = $request->getPost('is_radio');
        $data['is_local'] = $request->getPost('is_local');

        $data['is_outstation'] = $request->getPost('is_outstation');
        $data['is_bus'] = $request->getPost('is_bus');

        $getRequest = new Request ();
        $getRequest->setUri ( BUSINESSTAXIVAXIADDCOMPANY );
        $getRequest->setMethod ( 'POST' );
        $getRequest->setPost ( new Parameters ( $data ) );
        $client = new Client ();
        $response = $client->send($getRequest)->getBody();
        ...
        if ($r['error'] == ""){
            ...
            return $this->redirect()->toRoute('get-businessadmin-addcompany');
        }
        else{
            $m = $r['error'];
    ...
    public function BusinesstaxivaxiOperatorIssuesAction()
    {
        if(isset($_COOKIE['business_taxivaxi'])){
            $data['access_token'] = $_COOKIE['access_token_business_taxivaxi'];
            $data['type'] = 'operator_issue';
    ...
    public function getBusinesstaxivaxiRatesAction()
    {
        if(isset($_COOKIE['business_taxivaxi'])){
            $data['access_token'] = $_COOKIE['access_token_business_taxivaxi'];
            $data['user'] = 'Taxivaxi';
    ...
    public function BusinesstaxivaxiBookingsOldUnassignedAction()
    {
        if(isset($_COOKIE['business_taxivaxi'])){
            $data['access_token'] = $_COOKIE['access_token_business_taxivaxi'];
            $data['type'] = '5';
            ...
            if ($r['error'] == "" || $r['error'] == 'No Result Found'){
                ...
                $view = new ViewModel(array('mess'=>$m));
                $view->setTemplate('businesstaxivaxi/businesstaxivaxi/businesstaxivaxi-bookings-old-unassigned.phtml');
                return $view;
            }
        }
        else
    ...
    public function BusinesstaxivaxiBusBookingsOldUnassignedAction()
    {
        if(isset($_COOKIE['business_taxivaxi'])){
            $data['access_token'] = $_COOKIE['access_token_business_taxivaxi'];
            $data['type'] = '5';
            ...
            if ($r['error'] == "" || $r['error'] == 'No Result Found'){
                ...
                $view = new ViewModel(array('mess'=>$m));
                $view->setTemplate('businesstaxivaxi/businesstaxivaxi/businesstaxivaxi-bus-bookings-old-unassigned.phtml');
                return $view;
            }
        }
        else
    ...
    public function assignBusinesstaxivaxiBookingAction()
    {
        if(isset($_COOKIE['business_taxivaxi'])){
            ...
            if ($r['error'] == ""){
                return new ViewModel(array('taxis' => $response, 'booking_id' => $booking_id, 'operators' => $response2,));
            }
            else{
                $m = $r['error'];
    ...
    public function postBusinesstaxivaxiAssignAction()
    {
            ...
            $data['operator_id'] = $request->getPost('operator_id');
    ...
    public function assignBusinessradioBookingAction()
    {
        if(isset($_COOKIE['business_taxivaxi'])){
            ...
            $response2 = $client2->send($getRequest2)->getBody();
                ...
                return new ViewModel(array('booking_id' => $booking_id, 'operators' => $response2));
    ...
    public function sendDetailsBusinesstaxivaxiAction()
    {
        if(isset($_COOKIE['business_taxivaxi'])){
            $booking_id = $this->getEvent()->getRouteMatch()->getParam('booking_id');
            ...
            if ($r['error'] == ""){
                return new ViewModel(array('booking' => $response, 'operators' => $response2,));
            }
            else{
                $m = $r['error'];
    ...
    public function resendDetailsBusinesstaxivaxiAction()
    {
        if(isset($_COOKIE['business_taxivaxi'])){
            $booking_id = $this->getEvent()->getRouteMatch()->getParam('booking_id');
    ...
    public function postSendDetailsBusinesstaxivaxiAction()
    {
        if(isset($_COOKIE['business_taxivaxi'])){
            $request = $this->getRequest ();
            $booking_id = $this->getEvent()->getRouteMatch()->getParam('booking_id');
            ...
            $getRequest = new Request ();
            $getRequest->setUri ( ASSIGNOPERATOR );
            $getRequest->setMethod ( 'POST' );
            $getRequest->setPost ( new Parameters ( $data ) );
            $client = new Client ();
            $response = $client->send($getRequest)->getBody();
    ...
    public function addpostSendDetailsBusinesstaxivaxiAction()
    {
        if(isset($_COOKIE['business_taxivaxi'])){
            $request = $this->getRequest ();
            $booking_id = $this->getEvent()->getRouteMatch()->getParam('booking_id');
            ...
            $data ['operator_name'] = $request->getPost('operator_name');
            $data ['operator_contact'] = $request->getPost('operator_contact');
            $data ['operator_email'] = $request->getPost('operator_email');
            $data ['website'] = $request->getPost('website');
            $data ['contact_name'] = $request->getPost('contact_name');
            $data ['contact_no'] = $request->getPost('contact_no');
            $data ['contact_email'] = $request->getPost('contact_email');

            $getRequest = new Request ();
            $getRequest->setUri ( ADDASSIGNOPERATOR );
            $getRequest->setMethod ( 'POST' );
            $getRequest->setPost ( new Parameters ( $data ) );
            $client = new Client ();
            $response = $client->send($getRequest)->getBody();
    ...
    public function getReportOperatorIssueAction()
    {
        if(isset($_COOKIE['business_taxivaxi'])){
            $booking_id = $this->getEvent()->getRouteMatch()->getParam('booking_id');
    ...
    public function postReportOperatorIssueAction()
    {
        $data ['access_token'] = $_COOKIE['access_token_business_taxivaxi'];
        $request = $this->getRequest ();

        $data['booking_id'] = $request->getPost('booking_id');
        $data['operator_issue_description'] = $request->getPost('operator_issue_description');
        $data['operator_penalty'] = $request->getPost('operator_penalty');
        
        
        $getRequest = new Request ();
        $getRequest->setUri ( REPORTOPERATORISSUE );
        $getRequest->setMethod ( 'POST' );
        $getRequest->setPost ( new Parameters ( $data ) );
        $client = new Client ();
        $response = $client->send($getRequest)->getBody();
