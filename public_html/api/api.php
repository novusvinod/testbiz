<?php
	...
	//define("TAXIVAXI_NUMBER", "9582058187");

	 define("TAXIVAXI_EMAIL", "corporate.oe@taxivaxi.com");
	//define("TAXIVAXI_EMAIL", "chetan.b@novuslogic.co.in");
	...
	//----------------------------------COMPANY - Profile------------------------------//
				...
				if(mysqli_num_rows($sql) > 0)
				{
					...
					if($r['status'] == '')
					{
						...
						mysqli_query($this->db, "UPDATE bookings SET 
							external_reference_no='$crn', operator_name='OLA', driver_name='$driver_name',
			            ...
			            {	
							...
							sendEmail($booking_details['email'],$booking_details['reference_no']." - ".$company_name." Booking Assigned Successfully!!",$mail_body);
			            }
						...
						{	
							...
							sendEmail($resultt['people_email'],$booking_details['reference_no']." - ".$company_name." Booking Assigned Successfully!!",$mail_body);
			            }
			            
						//SMS to spoc
						$m = "Dear ".$booking_details['name'].",\n\nFor your booking with id ".$booking_details['reference_no'].".\nDriver: ".$driver_name." (".$driver_number.")\nTaxi: ".$car_model." (".$cab_number.")\n\nPlease call at ".TAXIVAXI_NUMBER." for any query.\n\nRgrds,\nTaxiVaxi";
		...
		private function assignRadioBooking()
		{
			...
			$sqlo = mysqli_query($this->db, "SELECT * 
				from operators
				where id = '$operator_id'");
			$resulto = mysqli_fetch_array($sqlo,MYSQL_ASSOC);
			$operator_name = $resulto['operator_name'];
			$operator_contact = $resulto['operator_contact'];
			$operator_email = $resulto['operator_email'];
			...
			{
				mysqli_query($this->db, "UPDATE bookings SET 
					external_reference_no='$reference_no', operator_id='$operator_id', 
					operator_name='$operator_name', operator_email='$operator_email', operator_contact='$operator_contact', 
				...
				if(!empty($username) and !empty($password))
				{
					if(filter_var($username, FILTER_VALIDATE_EMAIL))
					{
						...
						if(mysqli_num_rows($sql) > 0)
						{
							...
							$result = array('access_token' => $token,'admin'=>$result, 'admin_id' => $admin_id);
							$success = array('success' => "1", "error" => "","response"=>$result);
							$this->response($this->json($success), 200);
						}
						else
		...
		private function addCompany()
		{
			...
			$corporate_name = $this->_request['corporate_name'];
			$corporate_code = $this->_request['corporate_code'];
			$contact_name = $this->_request['contact_name'];
			$contact_no = $this->_request['contact_no'];

			$email = $this->_request['email'];
			$password = md5($this->_request['password']);
			$billing_name = $this->_request['billing_name'];
			$billing_address_line1 = $this->_request['billing_address_line1'];

			$billing_address_line2 = $this->_request['billing_address_line2'];
			$billing_address_line3 = $this->_request['billing_address_line3'];
			$has_auth_level = $this->_request['has_auth_level'];
			$has_billing_spoc_auth_level = $this->_request['has_billing_spoc_auth_level'];

			$has_billing_admin_auth_level = $this->_request['has_billing_admin_auth_level'];
			$has_assessment_codes = $this->_request['has_assessment_codes'];
			$is_radio = $this->_request['is_radio'];
			$is_local = $this->_request['is_local'];

			$is_outstation = $this->_request['is_outstation'];
			$is_bus = $this->_request['is_bus'];
			
			try
			{
				//Add new group
				mysqli_query($this->db,"INSERT into admins 
					(corporate_name,corporate_code,contact_name,contact_no,
						email,password,billing_name, billing_address_line1,
						billing_address_line2, billing_address_line3, has_auth_level, has_billing_spoc_auth_level,
						has_billing_admin_auth_level, has_assessment_codes, is_radio, is_local,
						is_outstation, is_bus,
						username) 
				VALUES ('$corporate_name','$corporate_code','$contact_name', '$contact_no',
				'$email','$password','$billing_name', '$billing_address_line1',
				'$billing_address_line2','$billing_address_line3','$has_auth_level', '$has_billing_spoc_auth_level',
				'$has_billing_admin_auth_level','$has_assessment_codes','$is_radio', '$is_local', 
				'$is_outstation', '$is_bus',
				...
				if($new_id)
				{
					...
					// sendEmail($auth_email,"[".$company_name."] Registered as Approver Level 2",$mail_body);
					
					$success = array('success' => "1", "error" => "","response"=> "SuccessFully Registered");
					$this->response($this->json($success), 200);
				}
				else
				{
	...
	//----------------------------------Bookings-Company Admin------------------------------//
		...
		private function getAdminBookingReport()
		{
			...
			$cq = mysqli_query($this->db, "SELECT corporate_code from admins WHERE id='$admin_id'");
			$cqr = mysqli_fetch_array($cq, MYSQL_ASSOC);
			$ref_no = 'TV'.$cqr['corporate_code'];
		...
		private function getAdminBusBookingReport()
		{
			...
			$cq = mysqli_query($this->db, "SELECT corporate_code from admins WHERE id='$admin_id'");
			$cqr = mysqli_fetch_array($cq, MYSQL_ASSOC);
			$ref_no = 'TV'.$cqr['corporate_code'].'BUS';
			...
			try
			{
				...
				if($type!=3)
				{
					$sql = mysqli_query($this->db, "SELECT 
						b.*,
						i.sub_total `bill_amount`, i.status `invoice_status`, i.previous_status `invoice_previous_status`, i.comment_taxivaxi `invoice_comment_taxivaxi`, i.comment_client `invoice_comment_client`, i.bill_id, i.accepted_by, i.rejected_by,
						p.people_cid `user_cid`, p.people_name `user_name`,
						a.corporate_code
						from bus_bookings `b`
						left join bus_invoice `i` on b.invoice_id = i.id
		...
		private function getAllTaxivaxiadminInvoices()
		{
			...
			{	
				...
				if($type!=3) //This can be 3 or 7
				{
					$sql = mysqli_query($this->db, "SELECT 
						b.*, 
						c.name `city_name`, tt.name `taxi_type_name`, tm.name `taxi_model_name`,
						i.sub_total `bill_amount`, i.status `invoice_status`, i.previous_status `invoice_previous_status`, i.comment_taxivaxi `invoice_comment_taxivaxi`, i.comment_client `invoice_comment_client`, i.bill_id, i.accepted_by, i.rejected_by,
						u.user_cid, u.user_name,
						a.corporate_code, a.corporate_name
						from bookings `b`
						left join user_bookings `ub` on ub.booking_id = b.id
						left join invoice `i` on b.invoice_id = i.id
						left join users `u` on ub.user_id = u.id
	...
	//----------------------------------GROUPS------------------------------//
		...
		private function addGroup()
		{
			...
			{
				//Add new group
				mysqli_query($this->db,"INSERT into groups (admin_id,group_name,budget,unallocated,is_radio,is_local,is_outstation, is_bus) VALUES ('$admin_id','$group_name','$budget', '$budget', '$is_radio', '$is_local', '$is_outstation', '$is_bus')");
				...
				if($new_id)
				{
					...
					$sql = mysqli_query($this->db, "SELECT contact_no, email, allocated, unallocated, corporate_name from admins where id = '$admin_id' ");
					...
					$company_name = $rlt['corporate_name'];
		...
		private function editGroup()
		{
			...
			try
			{
				...
				$query = mysqli_query($this->db,"UPDATE groups SET group_name='$group_name', budget='$budget', is_radio = '$is_radio', is_local = '$is_local', is_outstation = '$is_outstation', is_bus='$is_bus' WHERE id = $group_id");
		...
		private function getAllSubgroups()
		{
			...
			try
			{
				$sql = mysqli_query($this->db, "SELECT 
					...
					g.is_radio `group_is_radio`, g.is_outstation `group_is_outstation`, g.is_bus `group_is_bus`,
		...
		private function addSubgroup()
		{
			...
			{
				//Add new group
				mysqli_query($this->db,"INSERT into subgroups (admin_id,group_id,subgroup_name,budget,unallocated,is_radio,is_local,is_outstation, is_bus) VALUES ('$admin_id','$group_id','$subgroup_name','$budget', '$budget', '$is_radio', '$is_local','$is_outstation', '$is_bus')");
				...
				if($new_id)
				{
					...
					$sql = mysqli_query($this->db, "SELECT g.allocated, g.unallocated, a.corporate_name from groups `g` left join admins `a` on g.admin_id=a.id where g.id = '$group_id' ");
					$rlt = mysqli_fetch_array($sql, MYSQL_ASSOC);
					...
					$mail_body = sendRegistrationEmailToAuthenticator1($auth_name, $subgroup_name, $auth_email, 
						$auth_password, $rlt['contact_no'], $rlt['email']);
					sendEmail($auth_email,"[".$company_name."] Registered as Approver Level 1",$mail_body);
		...
		private function viewSubgroup()
		{
			...
			try
			{
				$sql = mysqli_query($this->db, "SELECT 
					sg.*, 
					g.is_radio `group_is_radio`, g.is_local `group_is_local`, g.is_outstation `group_is_outstation`, g.is_bus `group_is_bus`, g.unallocated `group_unallocated`, 
		...
		private function editSubgroup()
		{
			...
			try
			{
				...
				$query = mysqli_query($this->db,"UPDATE subgroups SET group_id = '$group_id',  subgroup_name='$subgroup_name', budget='$budget', is_radio = '$is_radio', is_local = '$is_local', is_outstation = '$is_outstation', is_bus = '$is_bus' WHERE id = '$subgroup_id'");
		...
		private function addSpoc()
		{
			...
			try
			{
				$sql_auth_check = mysqli_query($this->db, "SELECT has_auth_level from admins where id = '$admin_id' ");
				$rlt_auth_check = mysqli_fetch_array($sql_auth_check, MYSQL_ASSOC);
				$has_auth_level = intval($rlt_auth_check['has_auth_level']);
				if($has_auth_level == '1')
				{
					//Add new group
					mysqli_query($this->db,"INSERT into users (admin_id,subgroup_id,user_cid,user_name,user_contact,email,username,password,budget,status,is_radio,is_local,is_outstation, is_bus) VALUES ('$admin_id','$subgroup_id','$user_cid','$user_name', '$user_contact','$email', '$email', '$enc_pwd','$budget', '$status', '$is_radio', '$is_local','$is_outstation', '$is_bus')");

					$new_id = mysqli_insert_id($this->db);			
					if($new_id)
					{
						//Get group_id from subgroup_id
						$sql = mysqli_query($this->db, "SELECT s.group_id, a.corporate_name from subgroups `s` left join admins `a` on s.admin_id=a.id where s.id = '$subgroup_id' ");
						$rlt = mysqli_fetch_array($sql, MYSQL_ASSOC);
						...
						sendEmail($email,"[".$company_name."] Registered as Spoc",$mail_body);
				...
				{
					//Add Group
					mysqli_query($this->db,"INSERT into groups (admin_id,group_name,budget,unallocated,is_radio,is_local,is_outstation, is_bus) VALUES ('$admin_id','$user_name','0', '0', '$is_radio', '$is_local','$is_outstation', '$is_bus')");
					$new_id = mysqli_insert_id($this->db);			
					if($new_id)
					{			
						$result = array('group_id' => $new_id, 'message' => 'Group Added Successfully');
						...
						//Add new group Authenticator
						$sql  = mysqli_query($this->db, "SELECT id from group_authenticater where email = '$email' ");
						...
						{
							mysqli_query($this->db,"INSERT into group_authenticater (group_id, cid, name, contact_no, email, password, admin_id) VALUES ('$new_id','$user_cid','$user_name','$user_contact','$email', '$enc_pwd', '$admin_id')");
							$group_id = mysqli_insert_id($this->db);
						}
					...
					//ADD Subgroup
					$group_id = $new_id;
					mysqli_query($this->db,"INSERT into subgroups (admin_id,group_id,subgroup_name,budget,unallocated,is_radio,is_local,is_outstation, is_bus) VALUES ('$admin_id','$group_id','$user_name','0', '0', '$is_radio', '$is_local','$is_outstation', '$is_bus')");
					$new_id = mysqli_insert_id($this->db);
					if($new_id)
					{
						$budget=0;
						...
						//Add new group Authenticator
						$sql  = mysqli_query($this->db, "SELECT id from subgroup_authenticater where email = '$email' ");
						...
						{
							mysqli_query($this->db,"INSERT into subgroup_authenticater (subgroup_id, cid, name, contact_no, email, password, admin_id) VALUES ('$new_id','$user_cid','$user_name','$user_contact','$email', '$enc_pwd', '$admin_id')");
							$subgroup_id = mysqli_insert_id($this->db);
						}
						//Update Groups auath_id
						$query = mysqli_query($this->db,"UPDATE subgroups SET authenticater_id='$subgroup_id' WHERE id = $new_id");
					...
					//Add Spoc
					$subgroup_id = $new_id;
					mysqli_query($this->db,"INSERT into users (admin_id,subgroup_id,user_cid,user_name,user_contact,email,username,password,budget,status,is_radio,is_local,is_outstation, is_bus) VALUES ('$admin_id','$subgroup_id','$user_cid','$user_name', '$user_contact','$email', '$email', '$enc_pwd',0, '$status', '$is_radio', '$is_local','$is_outstation', '$is_bus')");
		...
		private function viewSpoc()
		{
			...
			try
			{
				$sql = mysqli_query($this->db, "SELECT e.*, sg.is_radio `subgroup_is_radio`, sg.is_local `subgroup_is_local`, sg.is_outstation `subgroup_is_outstation`, sg.is_bus `subgroup_is_bus`, sg.unallocated `subgroup_unallocated` from users `e` left join subgroups `sg` on e.subgroup_id = sg.id where e.id = '$employee_id'");
				if(mysqli_num_rows($sql) > 0)
				{
					$result = mysqli_fetch_array($sql,MYSQL_ASSOC);
		...
		private function editSpoc()
		{
			...
			try
			{
				...
				if($has_auth_level == '1')
				{
					...
					$query = mysqli_query($this->db,"UPDATE users SET user_cid = '$user_cid', subgroup_id = '$subgroup_id',  user_name='$user_name', email = '$email', budget='$budget', is_radio = '$is_radio', is_local = '$is_local',is_outstation = '$is_outstation', is_bus='$is_bus', status = '$status' WHERE id = '$user_id'");
					if($query)
					{
				...
				{
					$query = mysqli_query($this->db,"UPDATE users SET user_cid = '$user_cid', user_name='$user_name', email = '$email', user_contact = '$user_contact', status = '$status' WHERE id = '$user_id'");
					if($query)
					{
						$result = array('employee_id' => $user_id, 'message' => 'Employee Updated Successfully');					
						$success = array('success' => "1", "error" => "","response"=>$result);
...
//*********************************************************************************************************************//
			...
			try
			{
				...
				if(!empty($username) and !empty($password))
				{
					if(filter_var($username, FILTER_VALIDATE_EMAIL))
					{
						...
						$sql = mysqli_query($this->db, "SELECT u.*, a.has_auth_level, a.has_assessment_codes, a.has_billing_spoc_auth_level, a.has_billing_admin_auth_level from users `u` left join admins `a` on u.admin_id=a.id  where u.username = '$username' AND u.password = '$enc_pwd'");
		...
		public function addBooking()
		{
			...
			try
			{
				$sql_auth_check = mysqli_query($this->db, "SELECT has_auth_level FROM admins WHERE id='$admin_id'");
				$rlt_auth_check = mysqli_fetch_array($sql_auth_check, MYSQL_ASSOC);
				$has_auth_level = $rlt_auth_check['has_auth_level'];

				if($has_auth_level == '1')
				{
					$auth_1_status = '0';
					$auth_2_status = '0';
					$auth_accept_timing = '0000-00-00 00:00:00';
				}
				else
				{
					$auth_1_status = '1';
					$auth_2_status = '0';
					$auth_accept_timing = date();	
				}
				//Add Booking
				if($tour_type == '0')
				{
					$query = "INSERT into bookings (tour_type,pickup_location,drop_location,
						...
						status_auth1, status_auth2, approved_date) values ('$tour_type','$pickup_location', '$drop_location',
						now(),'$pickup_datetime',
						now(),now(),
						'$admin_id','$group_id', '$subgroup_id',
						'$ass_code', '$reason_booking', '$no_of_seats',
						'$auth_1_status', '$auth_2_status', '$auth_accept_timing')";
				...
				elseif ($tour_type == '1') 
				{
					...
					admin_id,group_id, subgroup_id,
					ass_code, reason_booking, no_of_seats,
					status_auth1, status_auth2, approved_date) values ('$tour_type','$city_id','$pickup_location',
				...
				if($booking_id)
				{
					$cq = mysqli_query($this->db, "SELECT corporate_code FROM admins WHERE id='$admin_id'");
					$cqr = mysqli_fetch_array($cq, MYSQL_ASSOC);
					$corporate_code = $cqr['corporate_code'];

					$reference_no = "TV".$corporate_code.$booking_id;
					//Add Reference No
		            try{
					...
					{	
						...
						sendEmail($resultt['people_email'],"$reference_no - New Booking ".$company_name,$mail_body);
		            }

		            $booking_details['booking_id'] = $reference_no;

		            //Communication to Spoc
		            $mail_body = bookingDetailsToSpoc($booking_details, $i, $user_details, $employee_details);
					sendEmail($booking_details['email'],"$reference_no - New Booking ".$company_name,$mail_body);

		            //Communication to Approver 1
		            $mail_body = bookingDetailsToAuthone($booking_details, $i, $user_details, $employee_details);
					sendEmail($resulta['email_approve_1'],"$reference_no - New Booking ".$company_name,$mail_body);

					//Communication to Approver 2
		            $mail_body = bookingDetailsToAuthtwo($booking_details, $i, $user_details, $employee_details);
					sendEmail($resulta['email_approver_2'],"$reference_no - New Booking ".$company_name,$mail_body);
			...
			try
			{
				...
				if(!empty($username) and !empty($password))
				{
					if(filter_var($username, FILTER_VALIDATE_EMAIL))
					{
						...
						$sql = mysqli_query($this->db, "SELECT sa.*, sg.budget, sg.is_radio, sg.is_local, sg.is_outstation, sg.is_bus, a.has_auth_level, a.has_assessment_codes, a.has_billing_spoc_auth_level, a.has_billing_admin_auth_level from subgroup_authenticater `sa` left join subgroups `sg` on sa.subgroup_id = sg.id left join admins `a` on sa.admin_id=a.id where sa.email = '$username' AND sa.password = '$enc_pwd' LIMIT 1");
		...
		private function acceptAuthoneBooking()
		{
			...
			try
			{
				...
				if($query)
				{
					...
					$booking_details['booking_id'] = $booking_details['reference_no'];
			...
			try
			{
				if(!empty($username) and !empty($password))
				{
					if(filter_var($username, FILTER_VALIDATE_EMAIL))
					{
						...
						$sql = mysqli_query($this->db, "SELECT ga.*, g.budget, g.is_radio, g.is_local, g.is_outstation, g.is_bus, a.has_auth_level, a.has_assessment_codes, a.has_billing_spoc_auth_level, a.has_billing_admin_auth_level from group_authenticater `ga` left join groups `g` on ga.group_id = g.id left join admins `a` on g.admin_id=a.id where ga.email = '$username' AND ga.password = '$enc_pwd' LIMIT 1");
				...
				if(!empty($username))
				{
					if(filter_var($username, FILTER_VALIDATE_EMAIL))
					{
						...
						if(mysqli_num_rows($sql) > 0)
						{
							...
							sendEmail($result['email'],"Password Reset!!",$mail_body);				
		...
		private function getAllTaxivaxiBookings()
		{
			...
			try
			{
				...
				switch($type)
				{
					case '1':  //Active (Unassigned)
					{
						$query = "SELECT b.*, g.group_name, a.corporate_name
						from bookings `b`
						left join groups `g` on b.group_id = g.id 
						left join admins `a` on g.admin_id = a.id where b.status_user = 1 and (b.status_auth1 = 1 OR b.status_auth2 = 1) and b.status_auth_taxivaxi < 2 and DATE_SUB(NOW(), INTERVAL 12 HOUR) < b.pickup_datetime order by pickup_datetime asc";
						break;
					}
					case '3':  //Archived Assigned
					{
						$query = "SELECT b.*, g.group_name, a.corporate_name
						from bookings `b`
						left join groups `g` on b.group_id = g.id 
						left join admins `a` on g.admin_id = a.id where b.status_user = 1 and (b.status_auth1 = 1 OR b.status_auth2 = 1) and b.status_auth_taxivaxi = 3 and b.is_invoice = 0 and b.has_operator_issue = 0 order by pickup_datetime desc";
						break;
					}
					case '5':  //Archived Unassigned
					{
						$query = "SELECT b.*, g.group_name, a.corporate_name
						from bookings `b`
						left join groups `g` on b.group_id = g.id 
						left join admins `a` on g.admin_id = a.id where b.status_user = 1 and (b.status_auth1 = 1 OR b.status_auth2 = 1) and b.status_auth_taxivaxi < 2 and b.is_invoice = 0 and DATE_SUB(NOW(), INTERVAL 12 HOUR) > b.pickup_datetime  order by pickup_datetime desc";
						break;
					}
					case '4': //Cancelled /Rejected
					{
						...
						from bookings `b`
						left join groups `g` on b.group_id = g.id 
						left join admins `a` on g.admin_id = a.id where b.status_user = 2 OR b.status_auth1 = 2 OR b.status_auth2 = 2 OR b.status_auth_taxivaxi = 2 order by pickup_datetime desc";
						break;
					}
					case '2': //Active (Assigned)
					{
						...
						from bookings `b`
						left join groups `g` on b.group_id = g.id 
						left join admins `a` on g.admin_id = a.id where b.status_user = 1 and (b.status_auth1 = 1 OR b.status_auth2 = 1) and b.status_auth_taxivaxi = 3 and b.is_invoice = 0 and DATE_SUB(NOW(), INTERVAL 12 HOUR) < b.pickup_datetime order by pickup_datetime asc";
						break;
					}
					case 'operator_issue':
					{
						...
						from bookings `b`
						left join groups `g` on b.group_id = g.id 
						left join admins `a` on g.admin_id = a.id where b.has_operator_issue = 1 ";
						break;
					}
				}
		...
		private function rejectTaxivaxiBooking()
		{
			...
			try
			{
				$query = mysqli_query($this->db,"UPDATE bookings set 
					status_auth_taxivaxi = '2' , cancel_reason = '$reason', rejected_date = now()
					where id = '$booking_id'");
				if($query)
				{
					...
					{	
						...
						sendEmail($resultt['people_email'],$booking_details['reference_no']." - ".$company_name." Booking Rejected!!",$mail_body);
		            }
		...
		private function taximodelsByBookingId()
		{
			...
			try
			{
				...
				if($taxi_type_id)
				{
					$sql = mysqli_query($this->db, "SELECT * from taxi_models where taxi_type_id = '$taxi_type_id'");
					if(mysqli_num_rows($sql) > 0)
					{
						$result = array();
						while($rlt = mysqli_fetch_array($sql, MYSQL_ASSOC))
				...
				{
					$sql = mysqli_query($this->db, "select * from taxi_models");
					if(mysqli_num_rows($sql) > 0)
					{
						$result = array();
						while($rlt = mysqli_fetch_array($sql, MYSQL_ASSOC))
		...
		private function assignTaxivaxiBooking()
		{
			...
			try
			{
				...
				driver_contact='$driver_contact', operator_id = '$operator_id', operator_name = '$operator_name',
				operator_contact = '$operator_contact', operator_email = '$operator_email',
		...
		private function reportOperatorIssue()
		{
			if($this->get_request_method() != "POST")
			{
				$response = array('success' => "0", 'error' => "Method Not Acceptable");
			...
			$booking_id = $this->_request['booking_id'];
			$operator_issue_description = $this->_request['operator_issue_description'];
			$operator_penalty = $this->_request['operator_penalty'];

			try
			{
				mysqli_query($this->db,"UPDATE bookings SET 
					operator_issue_description='$operator_issue_description',
					operator_penalty='$operator_penalty', 
					has_operator_issue=1
					where id='$booking_id'"
				);
		...
		private function addInvoiceRadio()
		{
			...
			try
			{
				$sql = mysqli_query($this->db,"SELECT a.has_billing_spoc_auth_level, a.has_billing_admin_auth_level from  bookings `b`  left join admins `a` on b.admin_id = a.id where b.id = '$booking_id'");
				$has_billing_spoc_auth_level;
				$has_billing_admin_auth_level;
				if(mysqli_num_rows($sql) > 0)
				{
					$result_a = mysqli_fetch_array($sql,MYSQL_ASSOC);
					$has_billing_spoc_auth_level = $result_a['has_billing_spoc_auth_level'];
					$has_billing_admin_auth_level = $result_a['has_billing_admin_auth_level'];
				}

				if($has_billing_spoc_auth_level == '0' && $has_billing_admin_auth_level == '0'){
				...
				elseif($has_billing_spoc_auth_level == '0' && $has_billing_admin_auth_level == '1') {
					mysqli_query($this->db,"INSERT into invoice (booking_id,tour_type,taxi_model_name,
						hours_done,allowed_hrs,extra_hours,hour_rate,
						extra_hours_charge,kms_done,allowed_kms,
						extra_kms,km_rate,extra_kms_charge,
		...
		private function getAllCities()
		{
			...
			$sql3 = mysqli_query($this->db, "SELECT admin_id FROM users where id='$employee_id'");
			$result3 = mysqli_fetch_array($sql3,MYSQL_ASSOC);
			$admin_id = $result3['admin_id'] ;
			
			try
			{
				$sql = mysqli_query($this->db, "select * from cities where id in (select city_id from rates where admin_id = '$admin_id')");
				if(mysqli_num_rows($sql) > 0)
				{
					$result = array();
					while($rlt = mysqli_fetch_array($sql, MYSQL_ASSOC))
		...
		public function addBusBooking()
		{
        	...
        	$query = "INSERT into bus_bookings (admin_id, group_id, subgroup_id,
				...
				booking_datetime, created, modified,
				status_auth1, status_auth2) values ('$admin_id', '$group_id', '$subgroup_id',
			...
			if (!mysqli_query($this->db,$query))
			  ...
			  $reference_no = "TV".$corporate_code."BUS".$booking_id;
	            try{
				...
				$user_details = $user_details.'Spoc Employee Id          : '.$booking_details['user_cid'].'<br>Name          : '.$booking_details['user_name'].'<br>Company Name          : '.$booking_details['corporate_name'].'<br>Email          : '.$booking_details['email'].'<br><br>';
		          ...
		          //Communication to Spoc
					...
					sendEmail($booking_details['email'],"New Booking - $ref_no",$mail_body);

		            //Communication to Approver 1
		            $mail_body = busBookingCreated($booking_details, $user_details, $employee_details,'approver1');
					sendEmail($booking_details['approver1_email'],"New Booking - $ref_no",$mail_body);

					//Communication to Approver 2
		            $mail_body = busBookingCreated($booking_details, $user_details, $employee_details,'approver2');
					sendEmail($booking_details['approver2_email'],"New Booking - $ref_no",$mail_body);
		...
		private function getAllTaxivaxiBusBookings()
		{
			...
			try
			{
				...
				switch($type)
				{
					case '1': //Active Unassigned
					{
						$query  = "SELECT b.*,u.user_cid,u.user_name,u.user_contact,u.email,p.people_cid,p.people_name,p.people_email,p.people_contact, a.corporate_name from bus_bookings `b` left join users `u` on b.user_id = u.id left join people `p` on b.people_id = p.id left join admins `a` on b.admin_id=a.id where ((b.status_auth1 = 1 or b.status_auth2 = 1) and b.status_spoc = 1 and b.status_auth_taxivaxi < 2) and b.is_assign = 0 and b.date_of_journey > DATE_SUB(NOW(), INTERVAL 24 HOUR) order by b.time_range asc";
						break;
					}
					case '3': //Archived Assigned
					{
						$query  = "SELECT b.*,u.user_cid,u.user_name,u.user_contact,u.email,p.people_cid,p.people_name,p.people_email,p.people_contact, a.corporate_name from bus_bookings `b` left join users `u` on b.user_id = u.id left join people `p` on b.people_id = p.id left join admins `a` on b.admin_id=a.id where b.status_spoc = 1 and (b.status_auth1 = 1 OR b.status_auth2 = 1) and b.status_auth_taxivaxi = 3 and b.is_invoice = 0  order by b.time_range asc";
						break;
					}
					case '5': //Archived Unassigned
					{
						$query  = "SELECT b.*,u.user_cid,u.user_name,u.user_contact,u.email,p.people_cid,p.people_name,p.people_email,p.people_contact, a.corporate_name from bus_bookings `b` left join users `u` on b.user_id = u.id left join people `p` on b.people_id = p.id left join admins `a` on b.admin_id=a.id where b.status_spoc = 1 and (b.status_auth1 = 1 OR b.status_auth2 = 1) and b.status_auth_taxivaxi < 2 and b.is_invoice = 0 and b.date_of_journey < DATE_SUB(NOW(), INTERVAL 24 HOUR) order by b.time_range asc";
						break;
					}
					case '4': //Rejected
					{
						$query  = "SELECT b.*,u.user_cid,u.user_name,u.user_contact,u.email,p.people_cid,p.people_name,p.people_email,p.people_contact, a.corporate_name from bus_bookings `b` left join users `u` on b.user_id = u.id left join people `p` on b.people_id = p.id left join admins `a` on b.admin_id=a.id where b.status_spoc = 2 OR b.status_auth1 = 2 OR b.status_auth2 = 2 OR b.status_auth_taxivaxi = 2  order by b.time_range asc";
						break;
					}
					case '2': //Active Assigned
					{
						$query  = "SELECT b.*,u.user_cid,u.user_name,u.user_contact,u.email,p.people_cid,p.people_name,p.people_email,p.people_contact, a.corporate_name from bus_bookings `b` left join users `u` on b.user_id = u.id left join people `p` on b.people_id = p.id left join admins `a` on b.admin_id=a.id where ((b.status_auth1 = 1 or b.status_auth2 = 1) and b.status_spoc = 1 and b.status_auth_taxivaxi = 3 and b.is_invoice = 0 and b.date_of_journey > DATE_SUB(NOW(), INTERVAL 24 HOUR)) order by b.time_range asc";
						break;
					}
		...
		private function assignTaxivaxiBusBooking()
		{
			...
			$boarding_point_taxivaxi = mysqli_escape_string($this->db,$this->_request['boarding_point_taxivaxi']);
			$pickup_datetime_taxivaxi = $this->_request['pickup_datetime_taxivaxi'];
			$ticket_number = mysqli_escape_string($this->db,$this->_request['ticket_number']);
			$pnr_number = mysqli_escape_string($this->db,$this->_request['pnr_number']);
			$bus_type_allocated = $this->_request['bus_type_allocated'];
			$operator_name = mysqli_escape_string($this->db,$this->_request['operator_name']);
		...
		public function assignOperator()
		{
			...
			$sqlo = mysqli_query($this->db, "SELECT * 
				...
				where id = '$operator_id'");
			$resulto = mysqli_fetch_array($sqlo,MYSQL_ASSOC);
			$name = $resulto['operator_name'];
			$email = $resulto['operator_email'];
			$contact_no = $resulto['operator_contact'];

			mysqli_query($this->db,"UPDATE bookings SET operator_id='$operator_id', operator_name = '$name',operator_email = '$email',operator_contact = '$contact_no' where id = '$booking_id'");
			...
			$sqll = mysqli_query($this->db, "SELECT 
				...
				where b.id = '$booking_id'");
			$resultl = mysqli_fetch_array($sqll,MYSQL_ASSOC);
			$r_no = $resultl['reference_no'];
			
			//Generete common varialble for email
			// $pickup_datetime = date();
			$booking_details['booking_id'] = $resultl['reference_no'];
			...
			sendEmail($email,"TaxiVaxi Booking | [".$booking_details['booking_id']."] | [".$booking_details['pickup_datetime']."] | [".$booking_details['city']."]",$mail_body);
		...
		public function addAndAssignOperator()
		{
			$token_id = $this->_request['access_token'];
			$auth_id = $this->checkTaxivaxiAdmin($token_id);
			...
			$operator_contact = $this->_request['operator_contact'];
			$operator_email = $this->_request['operator_email'];
			$website = $this->_request['website'];
			$contact_name = $this->_request['contact_name'];
			$contact_no = $this->_request['contact_no'];
			$contact_email = $this->_request['contact_email'];

			$query = "INSERT into operators (operator_name, operator_contact, operator_email,
						website,
						contact_name, contact_no, contact_email) 
							values 
						('$operator_name','$operator_contact','$operator_email',
					     '$website',
					     '$contact_name','$contact_no','$contact_email')";
			if (!mysqli_query($this->db,$query))
			  {
			...
			$operator_id = mysqli_insert_id($this->db);
		...
		private function addBusInvoice()
		{
			...
			try
			{
				mysqli_query($this->db,"UPDATE bus_bookings set 
					boarding_point_taxivaxi='$boarding_point_taxivaxi', pickup_datetime_taxivaxi='$pickup_datetime_taxivaxi', 
					ticket_number = '$ticket_number',
					pnr_number='$pnr_number', bus_type_allocated = '$bus_type_allocated',
					operator_name = '$operator_name', operator_contact = '$operator_contact', seat_number = '$seat_number' where id='$booking_id'"
				);

				$sql = mysqli_query($this->db,"SELECT a.has_billing_spoc_auth_level, a.has_billing_admin_auth_level from bus_bookings `b` left join admins `a` on b.admin_id = a.id where b.id = '$booking_id'");
				$has_billing_spoc_auth_level;
				$has_billing_admin_auth_level;
				if(mysqli_num_rows($sql) > 0)
				{
				...
				if($invoice_id)
				{
					...
					$result = array('rate_id' => $invoice_id, 'message' => 'Invoice Generated Successfully', 'invoice_id' => $invoice_id);	
					$success = array('success' => "1", "error" => "","response"=>$result);
					$this->response($this->json($success), 200);
				}
				else
		...
		private function editBusInvoice()
		{
			...
			try
			{
				...
				mysqli_query($this->db,"UPDATE bus_bookings set 
					...
					ticket_number = '$ticket_number',
					pnr_number='$pnr_number', bus_type_allocated = '$bus_type_allocated',
					operator_name = '$operator_name', operator_contact = '$operator_contact', seat_number = '$seat_number' where id IN (SELECT booking_id FROM bus_invoice WHERE id = '$invoice_id')"
				);
			...
			try
			{
				...
				if(mysqli_num_rows($sql) > 0)
				{
					...
					while($rlt = mysqli_fetch_array($sql, MYSQL_ASSOC))
					{
						$rlt2 = array();
						$booking_id = $rlt['id'];
						$rlt2['Booking ID'] = $rlt['reference_no'];
						switch($rlt['tour_type'])
						{
							case '0':
							{
		...
		private function spocClearInvoice()
		{
			...
			$sql = mysqli_query($this->db,"SELECT a.has_billing_spoc_auth_level, a.has_billing_admin_auth_level from invoice `i` left join bookings `b` on i.booking_id = b.id left join admins `a` on b.admin_id = a.id where i.id = '$invoice_id'");
			$has_billing_spoc_auth_level;
			$has_billing_admin_auth_level;
			if(mysqli_num_rows($sql) > 0)
			{
			...
			if($has_billing_spoc_auth_level == '1' && $has_billing_admin_auth_level == '0') {
				$query = mysqli_query($this->db,"UPDATE invoice SET previous_status=2, 
					status=6, accepted_by='$accepted_by', spoc_accept_time=now() WHERE id = $invoice_id");
			}
			else {
				$query = mysqli_query($this->db,"UPDATE invoice SET previous_status=status, 
		...
		private function spocClearBusInvoice()
		{
			...
			$sql = mysqli_query($this->db,"SELECT a.has_billing_spoc_auth_level, a.has_billing_admin_auth_level from bus_invoice `i` left join bus_bookings `b` on i.booking_id = b.id left join admins `a` on b.admin_id = a.id where i.id = '$invoice_id'");
			$has_billing_spoc_auth_level;
			$has_billing_admin_auth_level;
			if(mysqli_num_rows($sql) > 0)
			{
			...
			if($has_billing_spoc_auth_level == '1' && $has_billing_admin_auth_level == '0') {
				$query = mysqli_query($this->db,"UPDATE bus_invoice SET previous_status=2, 
				status=6, accepted_by='$accepted_by', spoc_accept_time=now() WHERE id = $invoice_id");
			}
			else {
				$query = mysqli_query($this->db,"UPDATE bus_invoice SET previous_status=status, 
		...
		public function invoiceStatusBilled()
		{
			...
			$bk_id = intval(explode(",", $booking_ids)[0]);
			$sql_query3 = "SELECT b.admin_id, a.corporate_code FROM bookings `b` left join admins `a` on b.admin_id=a.id WHERE b.id='$bk_id'";
			$sql3 = mysqli_query($this->db, $sql_query3);
			$result3 = mysqli_fetch_array($sql3,MYSQL_ASSOC);
			$admin_id = intval($result3['admin_id']);
			$corporate_code = $result3['corporate_code'];

			mysqli_query($this->db, "INSERT INTO bills(invoice_ids, booking_ids, bill_amount, bill_type, admin_id) VALUES ('$invoice_ids', '$booking_ids', '$sum_amount', '$tour_type', '$admin_id')");
			$bill_id = (string)mysqli_insert_id($this->db);
			$b_id = intval($bill_id);

			$bill_ref_no = $corporate_code."BILL".str_pad($b_id, 4, "0", STR_PAD_LEFT);
			$sql_query = "UPDATE bills SET bill_reference_no='".$bill_ref_no."' WHERE id=".$bill_id;
			...
			$success = array('success' => "1", "error" => "","response"=> $result['sub_total'], "bill_id"=>$bill_ref_no,);
			$this->response($this->json($success), 200);
		}
		...
		public function businvoiceStatusBilled()
		{
			...
			$bk_id = explode(",", $booking_ids)[0];
			$sql_query3 = "SELECT admin_id FROM bus_bookings WHERE id='$bk_id'";
			$sql3 = mysqli_query($this->db, $sql_query3);
			$result = mysqli_fetch_array($sql3,MYSQL_ASSOC);
			$admin_id = $result['admin_id'];

			mysqli_query($this->db, "INSERT INTO bills(invoice_ids, booking_ids, bill_amount, bill_type, admin_id) VALUES ('$invoice_ids', '$booking_ids', '$sum_amount', 3, '$admin_id')");
			$bill_id = (string)mysqli_insert_id($this->db);
		...
		private function getAllBills()
		{
			...
			try
			{
				...
				{
					$auth_id = $this->checkAdmin($token_id);
					$sql = mysqli_query($this->db, "SELECT b.*, a.corporate_code FROM bills `b` left join admins `a` on b.admin_id=a.id WHERE b.is_paid=$paid_status and b.admin_id='$auth_id'");
				}
				elseif($user == 'Taxivaxi')
				{
					$sql = mysqli_query($this->db, "SELECT b.*, a.corporate_code, a.corporate_name FROM bills `b` left join admins `a` on b.admin_id=a.id WHERE b.is_paid=$paid_status");
				}
		...
		private function getAllRates()
		{
			...
			try
			{
				...
				{
					$admin_id = $this->checkAdmin($token_id);
					$sql = mysqli_query($this->db, "SELECT r.*, c.name `city_name`, t.name `taxi_type` FROM rates `r` left join cities `c` on r.city_id=c.id left join taxi_types `t` on r.taxi_type_id = t.id WHERE r.admin_id='$admin_id' ");
				}
				elseif($user == 'Taxivaxi')
				{
					$admin_id = '';
					$sql = mysqli_query($this->db, "SELECT r.*, c.name `city_name`, t.name `taxi_type`, a.corporate_name FROM rates `r` left join cities `c` on r.city_id=c.id left join taxi_types `t` on r.taxi_type_id = t.id left join admins `a` on r.admin_id = a.id");
				}
				...
				if(mysqli_num_rows($sql) > 0)
				{
					...
					$result = array('access_token' => $token_id,'Rates'=>$result);
					$success = array('success' => "1", "error" => "","response"=> $result);
					$this->response($this->json($success), 200);
				} 
				else
		...
		private function getAllOperators()
		{
			...
			try
			{
				$sql = mysqli_query($this->db, "SELECT * FROM operators");
		...
		public function viewBill()
		{
			...
			$sql = mysqli_query($this->db, "SELECT b.*, a.billing_name, a.billing_address_line1, a.billing_address_line2, a.billing_address_line3, a.corporate_code FROM bills `b` left join admins `a` on b.admin_id=a.id WHERE b.id='$bill_id'");
			$result = mysqli_fetch_array($sql, MYSQL_ASSOC);
		...
		//Start Edit Invoice
		private function editInvoiceRadio()
		{
			if($this->get_request_method() != "POST")
			{
				$response = array('success' => "0", 'error' => "Method Not Acceptable");
			...
			try
			{
				mysqli_query($this->db,"UPDATE invoice SET 
					total='$total',
					taxivaxi_charge='$taxivaxi_charge', 
					taxivaxi_tax_rate='$taxivaxi_tax_rate', taxivaxi_tax_charge='$taxivaxi_tax_charge', 
					sub_total='$sub_total'
		...
		private function editInvoiceOutstation()
		{
			if($this->get_request_method() != "POST")
			{
				$response = array('success' => "0", 'error' => "Method Not Acceptable");
			...
			try
			{
				try
				{
					mysqli_query($this->db,"UPDATE invoice SET 
						...
						drop_date='$drop_date',drop_time='$drop_time',hours_done='$hours_done',allowed_hrs='$allowed_hrs',
						extra_hours='$extra_hours',hour_rate='$hour_rate',extra_hours_charge='$extra_hours_charge',
						start_km='$start_km',end_km='$end_km',kms_done='$kms_done',allowed_kms='$allowed_kms',
						extra_kms='$extra_kms',km_rate='$km_rate',extra_kms_charge='$extra_kms_charge',
						parking='$parking',driver='$driver',base_rate='$base_rate',total_ex_tax='$total_ex_tax',
						tax_rate='$tax_rate',tax='$tax',total='$total',
						taxivaxi_rate='$taxivaxi_rate',taxivaxi_charge='$taxivaxi_charge',
						taxivaxi_tax_rate='$taxivaxi_tax_rate',taxivaxi_tax_charge='$taxivaxi_tax_charge',
						sub_total='$sub_total' where id='$invoice_id'"
					);	
					$success = array('success' => "1", "error" => "","response"=>"Updated Successfully");
					$this->response($this->json($success), 200);
				}
				catch (Exception $e) 
				{
		...
		private function editInvoiceLocal()
		{
			if($this->get_request_method() != "POST")
			{
				$response = array('success' => "0", 'error' => "Method Not Acceptable");
 ...
 where access_token = '$token_id'");
		...
		private function getAllCodes()
		{
			...
			try
			{
				$sql = mysqli_query($this->db, "select id, assessment_code,code_desc from assessment_codes where admin_id='$admin_id'");
				if(mysqli_num_rows($sql) > 0)
				{
					$result = array();
					while($rlt = mysqli_fetch_array($sql, MYSQL_ASSOC))
					{
						$rlt['code_desc'] = $this->clean($rlt['code_desc']);
						array_push($result, $rlt);
					}
					$result2 = array('access_token' => $token_id,'AssCodes'=>$result);
					$success = array('success' => "1", "error" => "","response"=> $result2);
